...
class inHDGOSVWeather2(Converter, object):
	...
	def ConvertTime(self, time):
		if config.plugins.AtileHD.tempUnit.value == "Fahrenheit":
			...
			if len(time2) < 2:
				time2 = "0" + time2
			if time4 == "am":
				time = str(time1) + ":" + str(time2) + str(time4.replace("am", ""))
			else:
				time1 = int(time1) + 12
				time = str(time1) + ":" + str(time2) + str(time4.replace("pm", ""))
		return str(time)
...
#		time = "Fri, 29 Apr 2016 01:00 PM CEST" #								Debug source
	...
	def ConvertTemp(self, temp):
		if config.plugins.AtileHD.tempUnit.value == "Fahrenheit":
			...
			return str(int((float(temp) -32) * 5/9)) + "° C"

	def ConvertSpeed(self, speed):
		if config.plugins.AtileHD.tempUnit.value == "Fahrenheit":
			return str(int(float(speed) * 621371 / 1000000)) + " mpH"
#			return str(round((float(speed) * 621371 / 1000000), 1)) + " mpH" #		with decimal value
		else:
			return str(int(float(speed))) + " km/h"
#			return str(round(float(speed), 1)) + " km/h" #							with decimal value

	def ConvertVisibility(self, vision):
		if config.plugins.AtileHD.tempUnit.value == "Fahrenheit":
			return str(int(float(vision) * 621371 / 1000000)) + " miles"
#			return str(round(float(vision) * 621371 / 1000000), 1)) + " miles" #	with decimal value
		else:
			return str(int(float(vision))) + " km"
#			return str(round(float(vision), 1)) + " km" #							with decimal value
